...
BEGIN
    ...
    DECLARE cnt INT;
    DECLARE this_id INT;
    DECLARE this_gpa DECIMAL(3,2);

    
    SET cnt = 1 + CHAR_LENGTH(in_ids) - CHAR_LENGTH(REPLACE(in_ids, ',', ''));
    ...
    WHILE I <= cnt DO
        ...
        UPDATE Student
            SET Student.gpa = this_gpa
            WHERE Student.std_ID = this_id;
...
SET @gpa_list = ':gpa_list';

--invoking
CALL grades(@id_list, @gpa_list);
...
BEGIN
    ...
    --using cursor for reading the values
    DECLARE cur CURSOR FOR
        SELECT std_ID, gpa, credits
            FROM Student
            ORDER BY std_ID

    --when cursor runs out, declre true
    ...
    read_loop : LOOP
        FETCH cur INTO id, gpa, credits;
        IF done THEN
            LEAVE read_loop;
        END IF;

        --computing the cgpa
        SET cgpa = (gppa * credits) / credits;
        --Now determining if pass or fail
        IF cgpa >= 2.0 THEN
            SET status = 'PASS';
        ELSE
            SET status = 'FAIL';
